...
void renderNetworkTrafficHistorySection(
    ...
    const atm::NetworkPacketMtuMonitor &packet_mtu_monitor) {
  ...
  if (!series->aggregate) {
    ...
    // One compact wireless line for the selected interface when it is a
    // wireless device. The monitor is fed once per tick, so the render path
    // only reads already-cached state (no sysfs reads here). Wired and virtual
    // selections show nothing, and SSIDs/BSSIDs are never shown: the kernel
    // does not expose them via the approved native interfaces.
    const atm::NetworkWirelessInfo *wireless =
        wireless_monitor.tracked(series->identity);
    ...
    // One compact MTU/packet line for the selected interface when the packet
    // statistics monitor has retained data. Reads only cached monitor state;
    // never reads the network on the render path. The frame size is an
    // estimate (average bytes per frame from the aggregate byte/packet
    // counters), never a claimed size distribution.
    const atm::NetworkPacketMtuStats *packet_mtu =
        packet_mtu_monitor.tracked(series->identity);
    if (packet_mtu != nullptr) {
      const atm::NetworkPacketMtuSummary mtu_summary =
          atm::summarizeNetworkPacketMtu(
              *packet_mtu, packet_mtu_monitor.historyMaxSamples());
      if (mtu_summary.has_valid_data) {
        out << "Packet/MTU: "
            << atm::networkPacketMtuStateName(packet_mtu->state)
            << "   mtu "
            << (mtu_summary.current_mtu.has_value()
                    ? std::to_string(*mtu_summary.current_mtu)
                    : "unavailable")
            << "   rx_packets/s "
            << atm::formatNetworkPacketMtuRate(
                   mtu_summary.rates.rx_packets_per_second)
            << "   tx_packets/s "
            << atm::formatNetworkPacketMtuRate(
                   mtu_summary.rates.tx_packets_per_second)
            << "   avg_frame "
            << atm::formatNetworkPacketSizeEstimate(
                   mtu_summary.estimate.combined_bytes_per_frame)
            << '\n';
      }
    }
  }
...
const atm::FilesystemMonitor &filesystems,
  ...
  renderNetworkTrafficHistorySection(out, network_traffic_history,
                                     ...
                                     link_stats_monitor, packet_mtu_monitor);
...
void interactNetworkTrafficExport(const atm::NetworkTrafficHistory &traffic,
                                  ...
                                  const atm::NetworkPacketMtuMonitor &packet_mtu,
                                  ConsoleInput &input) {
  for (;;) {
    std::cout << "\033[2J\033[H";
    std::cout << "========================================\n"
    ...
    // Attach the hardware/driver, wireless, interface-info, link-state and
    // packet/MTU records for the selected series (all already-cached monitor
    // state — the export path never reads the network). The builder treats each
    // as optional, so a stale selection or a non-wireless interface simply
    // yields no wireless metadata.
    ...
    const atm::NetworkTrafficExportSnapshot snapshot =
        atm::buildNetworkTrafficExportSnapshot(
            *series, traffic.historyMaxSamples(),
            ...
            packet_mtu.tracked(effective_identity));
...
std::string buildNetworkInterfacePage(
  ...
  out << "\nPacket and MTU statistics\n";
  if (const atm::NetworkPacketMtuStats *packet_mtu =
          packet_mtu_monitor.tracked(identity);
      packet_mtu != nullptr) {
    out << atm::renderNetworkPacketMtu(*packet_mtu,
                                       monitor.historyMaxSamples());
    out << atm::renderNetworkPacketMtuChanges(*packet_mtu);

    atm::NetworkPacketMtuChartConfig chart;
    chart.data_width = 40;
    chart.data_height = 5;
    out << "  rx packets/s trend:\n"
        << atm::renderNetworkPacketMtuTrend(
               *packet_mtu, atm::NetworkPacketMtuTrendMetric::RxPacketRate,
               chart)
        << '\n';
    out << "  tx packets/s trend:\n"
        << atm::renderNetworkPacketMtuTrend(
               *packet_mtu, atm::NetworkPacketMtuTrendMetric::TxPacketRate,
               chart)
        << '\n';
    out << "  estimated average frame size trend:\n"
        << atm::renderNetworkPacketMtuTrend(
               *packet_mtu,
               atm::NetworkPacketMtuTrendMetric::CombinedAvgFrameSize, chart)
        << '\n';
  } else {
    out << "  not sampled yet\n";
  }
  ...
  out << "\nWireless\n";
  const atm::NetworkWirelessInfo *wireless = wireless_monitor.tracked(identity);
  if (wireless == nullptr || wireless->presence == atm::WirelessPresence::Unknown) {
    // No judgement absorbed yet (or the monitor has not seen the interface);
    // fall back to the raw snapshot signal so the page is never blank.
    if (!info.wireless.present) {
      out << "  not a wireless interface\n";
    } else {
      out << "  status: not sampled yet\n";
    }
  } else if (wireless->presence == atm::WirelessPresence::NotWireless) {
    appendLabeled(out, "  Status:",
                  "not a wireless interface (no native wireless metadata)");
  } else {
    appendLabeled(out, "  Status:", "wireless interface");
    std::string association = atm::wirelessAssociationName(wireless->association);
    if (wireless->association == atm::WirelessAssociation::Associated) {
      association += " (kernel carrier bit set)";
    } else if (wireless->association ==
               atm::WirelessAssociation::Disconnected) {
      association += " (no carrier)";
    } else if (wireless->association == atm::WirelessAssociation::Unknown) {
      association = "unknown (no carrier bit exposed)";
    }
    appendLabeled(out, "  Association:", association);
    if (wireless->is_mac80211) {
      std::string phy = wireless->phy_name.empty() ? "unavailable"
                                                   : wireless->phy_name;
      if (wireless->phy_index.has_value()) {
        phy += " (index " + std::to_string(*wireless->phy_index) + ")";
      }
      appendLabeled(out, "  PHY:", phy);
    }
    const std::string phy_state =
        atm::wirelessFieldStateName(wireless->phy_state);
    if (phy_state != "unknown") {
      appendLabeled(out, "  PHY state:", phy_state);
    }
    appendLabeled(out, "  Signal:",
                  atm::formatWirelessSignal(wireless->signal_dbm));
    appendLabeled(out, "  Noise:",
                  atm::formatWirelessNoise(wireless->noise_dbm));
    appendLabeled(
        out, "  Link quality:",
        atm::formatWirelessLinkQuality(wireless->link_quality,
                                       wireless->is_mac80211));
    appendLabeled(out, "  Enabled:", wireless->enabled ? "yes" : "no");
    appendLabeled(out, "  Carrier:",
                  wireless->carrier_exposed
                      ? (wireless->has_carrier ? "yes" : "no carrier")
                      : "not exposed (association cannot be confirmed)");
    std::string field_state = atm::wirelessFieldStateName(wireless->field_state);
    if (wireless->field_state == atm::WirelessFieldState::Stale) {
      field_state += " (last valid values preserved)";
    }
    appendLabeled(out, "  Values:", field_state);
    if (wireless->last_read != std::chrono::system_clock::time_point{}) {
      appendLabeled(out, "  Last update:",
                    formatTimestamp(wireless->last_read));
    }
...
int main() {
  ...
  atm::NetworkPacketMtuMonitor network_packet_mtu;
  ...
  network_link_stats.setHistoryMaxSamples(
      static_cast<std::size_t>(settings.settings().history.max_samples));
  network_packet_mtu.setHistoryMaxSamples(
      static_cast<std::size_t>(settings.settings().history.max_samples));
  ...
  network_packet_mtu.update(network_interface_details.current());
      ...
      case ConsoleInput::Command::InspectNetwork:
        if (view == ViewMode::List) {
          interactNetworkDetail(network, network_interface_details,
                                ...
                                network_link_stats, network_packet_mtu, input);
        }
        break;
